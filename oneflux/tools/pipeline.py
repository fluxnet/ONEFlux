...
def run_pipeline(datadir,
    ...
    try:
        pipeline = Pipeline(siteid=siteid,
                    ...
                    qc_auto_execute=steps.get('qc_auto_execute', False),
                    ustar_mp_execute=steps.get('ustar_mp_execute', False),
                    ustar_cp_execute=steps.get('ustar_cp_execute', False),
                    meteo_era_execute=steps.get('meteo_era_execute', False), #(False if era_source_dir is None else True)),
                    meteo_proc_execute=steps.get('meteo_proc_execute', False),
                    nee_proc_execute=steps.get('nee_proc_execute', False),
                    energy_proc_execute=steps.get('energy_proc_execute', False),
                    nee_partition_nt_execute=steps.get('nee_partition_nt_execute', False),
                    nee_partition_dt_execute=steps.get('nee_partition_dt_execute', False),
                    prepare_ure_execute=steps.get('prepare_ure_execute', False),
                    ure_execute=steps.get('ure_execute', False),
