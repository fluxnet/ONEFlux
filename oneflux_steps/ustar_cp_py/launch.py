...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:18
    ...
    if input_folder[length(input_folder)] != '\\' and input_folder[length(input_folder)] != '/':
...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:30
    ...
    if output_folder[length(output_folder)] != '\\' and output_folder[length(output_folder)] != '/':
...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:168
        ...
        fclose(fid)
        clear('i','temp','r','fid')
        imported_data=importdata(concat([input_folder,d[n].name]),',',(9 + length(notes)))
...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:182
        for y in arange(1,length(header[9 + length(notes),arange()])).reshape(-1):
            for i in arange(1,numel(input_columns_names)).reshape(-1):
                if logical_or((strcmpi(header[(9 + length(notes)),y],input_columns_names[i])),(strcmpi(header[(9 + length(notes)),y],strcat('itp',input_columns_names[i])))):
...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:309
        ...
        Cp2,Stats2,Cp3,Stats3=cpdBootstrapUStarTh4Season20100901(t,NEE,uStar,T,fNight,fPlot,cSiteYr,nBoot,nargout=4)
# oneflux_steps/ustar_cp_refactor_wip/launch.m:316
        Cp,n,tW,CpW,cMode,cFailure,fSelect,sSine,FracSig,FracModeD,FracSelect=cpdAssignUStarTh20100901(Stats2,fPlot,cSiteYr,nargout=11)
...
# oneflux_steps/ustar_cp_refactor_wip/launch.m:327
            dlmwrite(concat([output_folder,char(site),'_uscp_',char(year),'.txt']),Cp,'precision',8)
            fid=fopen(concat([output_folder,char(site),'_uscp_',char(year),'.txt']),'a')
# oneflux_steps/ustar_cp_refactor_wip/launch.m:330
            ...
            error_str=matlabarray(concat([[error_str],[char(site),'_uscp_',char(year),' ',cFailure]]))
