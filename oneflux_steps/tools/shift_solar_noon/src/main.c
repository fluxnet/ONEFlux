...
static int graph_output = 1;										/* default is on */
...
static const char notes[] = "processed on %s with shift_solar_noon "PROGRAM_VERSION" compiled using "COMPILER" on "__DATE__" at " __TIME__;
const char filter[] = "*.*";
...
int ustar_from_tau(DATASET *const dataset) {
	...
	if ( -1 == USTAR ) {
		USTAR = DATASET_ADD_VAR(dataset, var_names[USTAR_INPUT]);
		if ( -1 == USTAR ) {
...
int set_nee(DATASET *const dataset) {
	...
	if ( NEE != -1 ) {
		...
		NEE = DATASET_ADD_VAR(dataset, var_names[NEE_INPUT]);
		if ( -1 == NEE ) {
...
static int set_vpd(DATASET *const dataset) {
	...
	/* add column */
	VPD = DATASET_ADD_VAR(dataset, var_names[set_itp ? itpVPD_INPUT : VPD_INPUT]);
	if ( -1 == VPD ) {
...
static void check_for_neg(DATASET *const dataset, const char *const var) {
	...
	for ( i = 0; i < dataset->rows_count; i++ ) {
		flag = IS_INVALID_VALUE(dataset->rows[i].value[VAR]);
		if ( flag || (dataset->rows[i].value[VAR] < 0)  ) {
		...
		/*
		if ( !IS_INVALID_VALUE(dataset->rows[i].value[VAR]) && (dataset->rows[i].value[VAR] < 0) ) {
			if ( ARE_FLOATS_EQUAL(0, dataset->rows[i].value[RPOT]) ) {
				dataset->rows[i].value[VAR] = 0;
			} else {
				dataset->rows[i].value[VAR] = INVALID_VALUE;
...
static int set_swin_from_ppfd(DATASET *const dataset) {
	...
	if ( !valid_swin ) {
		if ( -1 == SWIN ) {
			SWIN = DATASET_ADD_VAR(dataset, var_names[SWIN_INPUT]);
			if ( -1 == SWIN ) {
			...
			for ( i = 0; i < dataset->rows_count; i++ ) {
				if ( !IS_INVALID_VALUE(dataset->rows[i].value[PPFD]) ) {
					/* update value */
					dataset->rows[i].value[SWIN] = dataset->rows[i].value[PPFD] * 0.52;
					/* update flag */
					dataset->flags[i].value[SWIN_FROM_PPFD] = 1;
					/* update missings */
...
static char *timestamp_for_sr(int row, int yy, const int hourly) {
	...
	sprintf(buffer, "%02,%02d,%04d,%02d,%02d", t->MM, t->DD, t->YYYY, t->hh, t->mm); 
...
static int write_dd_with_notes(FILE *const f, const DATASET *const dataset) {
	...
	for ( i = 0; i < dataset->columns_count; i++ ) {
		if ( dataset->header[i].y ) {
			...
			sprintf(buffer, "\nnotes,var %s was renamed in %s", var_name_f(&dataset->header[i]), dataset->header[i].name);
...
static int save_solar_file(DATASET *const dataset) {
	...
	for ( doy_loop = 10; doy_loop < (dataset->leap_year ? 366 : 365); doy_loop += 20 ) {
		...
		/* check rpots */
		for ( i = 0; i < dataset->rows_count; i++ ) {
			if ( !ARE_FLOATS_EQUAL(dataset->rows[i].value[RPOT], rpots[i]) ) {
				puts("rpot differs from solar noon rpot.");
				free(periods);
				free(rpots);
				fclose(f);
				return 0;
