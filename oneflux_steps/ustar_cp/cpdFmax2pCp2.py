...
# ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m
...
def cpdFmax2pCp2(Fmax=None, n=None, *args, **kwargs):
    ...
    p = copy(NaN)
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:21
    if logical_or(logical_or(isnan(Fmax), isnan(n)), n) < 10:
        return p

    pTable = concat([0.8, 0.9, 0.95, 0.99]).T
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:23
    np = length(pTable)
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:23
    nTable = concat([10, 15, 20, 30, 50, 70, 100, 150, 200, 300, 500, 700, 1000]).T
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:24
    ...
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:25
    FmaxCritical = []
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:40
    ...
    if Fmax < FmaxCritical(1):
        ...
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:42
        p = dot(2, (1 - fcdf(fAdj, 3, n)))
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:43
        if p > 1:
            p = 1
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:44
        return p

    if Fmax > FmaxCritical(end()):
        fAdj = dot(finv(0.995, 3, n), Fmax) / FmaxCritical(3)
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:47
        p = dot(2, (1 - fcdf(fAdj, 3, n)))
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:48
        if p < 0:
            p = 0
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:49
        return p

    p = interp1(FmaxCritical, 1 - pTable, Fmax, "pchip")


# ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp2.m:51
