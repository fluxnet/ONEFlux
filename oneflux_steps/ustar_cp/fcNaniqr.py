...
# ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m
...
def fcnaniqr(X=None, *args, **kwargs):
    ...
    d = size(X)
    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:32
    d = setdiff(d, 1)
    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:32
    nd = length(d)
    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:32
    if 1 == nd:
        ...
        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:36
        nYaN = length(iYaN)
        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:36
        IQR = copy(NaN)
        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:36
        if nYaN <= 3:
            y = X(iYaN)
            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:38
            yN = prctile(y, 25)
            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:38
            yX = prctile(y, 75)
            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:38
    ...
    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:38
        ...
        if 2 == nd:
            ...
            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:41
            IQR = dot(NaN, ones(1, nc))
            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:41
            for ic in arange(1, nc).reshape(-1):
                iYaN = find(logical_not(isnan(X(arange(), ic))))
                # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:43
                nYaN = length(iYaN)
                # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:43
                if nYaN > 3:
                    y = X(iYaN, ic)
                    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:45
                    yN = prctile(y, 25)
                    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:45
                    yX = prctile(y, 75)
                    # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:45
        ...
        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:45
            ...
            if 3 == nd:
                ...
                # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:49
                IQR = dot(NaN, ones(nc, nq))
                # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:49
                for iq in arange(1, nq).reshape(-1):
                    for ic in arange(1, nc).reshape(-1):
                        iYaN = find(logical_not(isnan(X(arange(), ic, iq))))
                        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:52
                        nYaN = length(iYaN)
                        # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:52
                        if nYaN > 3:
                            y = X(iYaN, ic, iq)
                            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:54
                            yN = prctile(y, 25)
                            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:54
                            yX = prctile(y, 75)
                            # ../ONEFlux/oneflux_steps/ustar_cp/fcNaniqr.m:54
