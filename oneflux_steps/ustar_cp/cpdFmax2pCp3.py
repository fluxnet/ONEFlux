...
# ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m


@function
def cpdFmax2pCp3(Fmax=None, n=None, *args, **kwargs):
    varargin = cpdFmax2pCp3.varargin
    nargin = cpdFmax2pCp3.nargin
    ...
    p = copy(NaN)
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:21
    if logical_or(logical_or(isnan(Fmax), isnan(n)), n) < 10:
        return p

    pTable = concat([0.9, 0.95, 0.99]).T
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:23
    nTable = concat([arange(10, 100, 10), arange(150, 600, 50), 800, 1000, 2500]).T
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:24
    FmaxTable = concat(
        [
    ...
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:25
    FmaxCritical = []
    # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:49
    ...
    if Fmax < FmaxCritical(1):
        ...
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:51
        p = dot(2, (1 - fcdf(fAdj, 3, n)))
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:52
        if p > 1:
            p = 1
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:53
        return p

    if Fmax > FmaxCritical(3):
        fAdj = dot(finv(0.995, 3, n), Fmax) / FmaxCritical(3)
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:56
        p = dot(2, (1 - fcdf(fAdj, 3, n)))
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:57
        if p < 0:
            p = 0
        # ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:58
        return p

    p = interp1(FmaxCritical, 1 - pTable, Fmax, "pchip")


# ../ONEFlux/oneflux_steps/ustar_cp/cpdFmax2pCp3.m:60
